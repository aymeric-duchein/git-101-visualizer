...
var js_1 = require("@gitgraph/js");
var current = 0;
var graphContainer = document.getElementById("gitgraph");
var graphContainer2 = document.getElementById("gitgraph2");
graphContainer.style.opacity = '0';
graphContainer2.style.opacity = '1';
graphContainer.addEventListener('scroll', function (event) {
    console.log(event);
    graphContainer2.scrollTop = graphContainer.scrollTop;
});
graphContainer2.addEventListener('scroll', function (event) {
...
function fadeIn1() {
    if (parseFloat(graphContainer.style.opacity) < 1) {
        ...
        setTimeout(function () { return fadeIn1(); }, 25);
    }
    else {
        fadeOut2();
    }
...
function fadeOut1() {
    if (parseFloat(graphContainer.style.opacity) > 0) {
        graphContainer.style.opacity = (parseFloat(graphContainer.style.opacity) - 0.1) + '';
        setTimeout(function () { return fadeOut1(); }, 25);
    }
    else {
...
function fadeIn2() {
    if (parseFloat(graphContainer2.style.opacity) < 1) {
        ...
        setTimeout(function () { return fadeIn2(); }, 25);
    }
    else {
        fadeOut1();
    }
...
function fadeOut2() {
    if (parseFloat(graphContainer2.style.opacity) > 0) {
        graphContainer2.style.opacity = (parseFloat(graphContainer2.style.opacity) - 0.1) + '';
        setTimeout(function () { return fadeOut2(); }, 25);
    }
    else {
...
setInterval(function () {
    var gitgraph;
    if (current === 1) {
        gitgraph = js_1.createGitgraph(graphContainer);
    }
    else {
        gitgraph = js_1.createGitgraph(graphContainer2);
    }
    var req = new XMLHttpRequest();
    req.open('GET', 'http://localhost:3000/', false);
    req.send(null);
    if (req.status === 200) {
        var logs = req.responseText;
